 ...
 *         Yashul Aggarwal
 * 
 * @version 4.30	2018-03-12 The JavaFX-based GUI for the implementation of a Double Calculator with Error Term
 * 
 */
...
public class UserInterface {
	...
	**********************************************************************************************/
	...
	private Label label_errOperand1ErrorTerm = new Label("");                // Label to display specific 
	private Label label_errOperand2ErrorTerm = new Label("");                // error messages
	private Label label_errOperand1ErrorTermx = new Label("");               // Label to display a error message 
	private Label label_errOperand2ErrorTermx = new Label("");	            // when user tries to perform any function 
    ...
    private TextFlow ErrorTerm1;
    private Text errOperand1Part1 = new Text();
    private Text errOperand1Part2 = new Text();
    private TextFlow ErrorTerm2;
    private Text errOperand2Part1 = new Text();
    private Text errOperand2Part2 = new Text();
	...
	**********************************************************************************************/
	...
	public UserInterface(Pane theRoot) {
		...
		setupLabelUI(label_PlusminusOperand1, "Arial", 24, Calculator.WINDOW_WIDTH-10, Pos.BASELINE_LEFT, 730, 70);		
		
		// Establish an error message for the first operand error term just below it, left aligned
		setupLabelUI(label_errOperand1ErrorTermx, "Arial", 18, Calculator.WINDOW_WIDTH-10, Pos.BASELINE_LEFT, 400, 45);
		label_errOperand1ErrorTermx.setTextFill(Color.RED);					
		
		// Label the error term first operand just above it, left aligned
		setupLabelUI(label_Operand1ErrorTerm, "Arial", 18, Calculator.WINDOW_WIDTH-10, Pos.BASELINE_LEFT, 760, 40);
		...
		// process both fields to ensure that we are ready to perform as soon as possible.
		setupTextUI(text_Operand1ErrorTerm, "Arial", 18, Calculator.WINDOW_WIDTH-800, Pos.BASELINE_LEFT, 760, 70, true);
		...
		// process both fields to ensure that we are ready to perform as soon as possible.
		setupTextUI(text_Operand2, "Arial", 18, Calculator.WINDOW_WIDTH-400, Pos.BASELINE_LEFT, 10, 180, true);
		...
		// Establish an error message for the second operand just below it, left aligned		
		...
		setupLabelUI(label_PlusminusOperand2, "Arial", 24, Calculator.WINDOW_WIDTH-10, Pos.BASELINE_LEFT, 730, 180);
		
		// Label the error term second operand error term just above it, left aligned
		setupLabelUI(label_Operand2ErrorTerm, "Arial", 18, Calculator.WINDOW_WIDTH-10, Pos.BASELINE_LEFT, 760, 155);
		
		// Establish the error term text input operand field and when anything changes in operand 2,
		// process both fields to ensure that we are ready to perform as soon as possible.
		setupTextUI(text_Operand2ErrorTerm, "Arial", 18, Calculator.WINDOW_WIDTH-800, Pos.BASELINE_LEFT, 760, 180, true);
		...
		setupLabelUI(label_errOperand2ErrorTermx, "Arial", 18, Calculator.WINDOW_WIDTH-10, Pos.BASELINE_LEFT, 400, 155);
		label_errOperand2ErrorTermx.setTextFill(Color.RED);
		...
		setupTextUI(text_Result, "Arial", 18, Calculator.WINDOW_WIDTH-400, Pos.BASELINE_LEFT, 10, 290, false);
		
		// Label the error result just above the result output field, left aligned
		setupLabelUI(label_ResultErrorTerm, "Arial", 18, Calculator.WINDOW_WIDTH-10, Pos.BASELINE_LEFT, 760, 260);
				
		// Establish the error result output field.  It is not editable, so the text can be selected and copied, 
		// but it cannot be altered by the user.  The text is left aligned.
		setupTextUI(text_ResultErrorTerm, "Arial", 18, Calculator.WINDOW_WIDTH-800, Pos.BASELINE_LEFT, 760, 290, false);
		
		// Establish the plus minus sign between the text fields of Result and its error term, left aligned
		setupLabelUI(label_PlusminusResult, "Arial", 24, Calculator.WINDOW_WIDTH-10, Pos.BASELINE_LEFT, 730, 290);
		...
		// Error Message for the Error Term for operand 1
			    ...
			    errOperand1Part2.setFill(Color.RED);
				...
				ErrorTerm1.setLayoutX(Calculator.WINDOW_WIDTH/2+63);  
				ErrorTerm1.setLayoutY(100);
				
		// Error Message for the Error Term for operand 2
				errOperand2Part1.setFill(Color.BLACK);
			    errOperand2Part1.setFont(Font.font("Arial", FontPosture.REGULAR, 18));
			    errOperand2Part2.setFill(Color.RED);
			    errOperand2Part2.setFont(Font.font("Arial", FontPosture.REGULAR, 24));
			    ErrorTerm2 = new TextFlow(errOperand2Part1, errOperand2Part2);
				...
				ErrorTerm2.setLayoutX(Calculator.WINDOW_WIDTH/2+63);  
				ErrorTerm2.setLayoutY(100);
				
		// Place all of the just-initialized GUI elements into the pane
		theRoot.getChildren().addAll(label_DoubleCalculator, label_Operand1, text_Operand1, label_Operand1ErrorTerm, text_Operand1ErrorTerm,
				label_errOperand1, 	label_Operand2, text_Operand2, label_Operand2ErrorTerm, text_Operand2ErrorTerm, label_errOperand2, 
				label_Result, text_Result, label_ResultErrorTerm, text_ResultErrorTerm, label_errResult, button_Add, button_Sub, button_Mpy, 
				button_Div, button_Sqrt, err1, err2, label_errOperand1x, label_errOperand2x, label_PlusminusOperand1,
	 ...
	 * message when the user enters the value of operand1 and operand1 error term
	 * 
	 */
	public void message1() {
		String errMessage = CalculatorValue.checkMeasureValue(text_Operand1.getText());
		String errMessageErrorTerm = CalculatorValue.checkErrorTerm(text_Operand1ErrorTerm.getText());
		if (errMessage != "") {
	...
	public void message2() {
		...
		String errMessageErrorTerm = CalculatorValue.checkErrorTerm(text_Operand2ErrorTerm.getText());
		if (errMessage != "") {
			...
			if (CalculatorValue.errorTermIndexofError <= -1) return;
			String input = CalculatorValue.errorTermInput;
			errOperand2Part1.setText(input.substring(0, CalculatorValue.errorTermIndexofError));
			errOperand2Part2.setText("\u21EB");
		}
	  }
	}
	...
	**********************************************************************************************/
	...
	private void setOperand1ErrorTerm() {
		...
		label_Result.setText("Result");						// the result, so we clear the old result.
		label_errResult.setText("");
		if (perform.setOperand1(text_Operand1ErrorTerm.getText())) {	// Set the operand and see if there was an error
			label_errOperand1ErrorTerm.setText("");					// If no error, clear this operands error
			label_errOperand1ErrorTermx.setText("");
			errOperand1Part1.setText("");                   // Clear the first term of error part
			errOperand1Part2.setText("");                   // Clear the second term of error part
			if (text_Operand2.getText().length() == 0)		// If the other operand is empty, clear its error
				label_errOperand2.setText("");				// as well.
		}
		else 												// If there's a problem with the operand, display
	...
	/**********
	 * This is the divide routine.  If the divisor is zero, the result is declared to be invalid.
	 * 
	 */
	...
	/**********
	 * This is the square root routine. The result will not depend on the value entered in operand 2.
	 * 
	 */
